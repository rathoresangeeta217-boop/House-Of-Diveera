    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      try {
        if (!snapshot.empty) {
          ...
          // If no products marked as isBigDeal, show random selection for variety
          try {
            ...
            if (!fallbackSnapshot.empty) {
              ...
              setProducts(shuffle(FALLBACK_PRODUCTS.filter(p => p.isBigDeal || p.price < 2000)).slice(0, 4));
            }
          } catch (err) {
            setProducts(shuffle(FALLBACK_PRODUCTS.filter(p => p.isBigDeal || p.price < 2000)).slice(0, 4));
          }
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-6 md:mb-8">
          <h2 className="text-xl md:text-2xl font-sans font-black text-diveera-dark flex items-center gap-2">
            ...
            Big <span className="text-red-600 relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-[2px] after:bg-red-600">Deals</span>
          </h2>
